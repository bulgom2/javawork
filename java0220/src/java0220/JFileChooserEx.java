...
public class JFileChooserEx extends JFrame {
		...
		if (btnFileOpen == null) {
			...
			btnFileOpen.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					jFileChooser.addChoosableFileFilter(new FileNameExtensionFilter(
							"텍스트 파일(*.txt)", "txt"));
					int option = jFileChooser.showOpenDialog(JFileChooserEx.this);
					//어떤 버튼을 클릭했는지 확인하기
		...
		if (btnFileSave == null) {
			...
			btnFileSave.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					jFileChooser.addChoosableFileFilter(new FileNameExtensionFilter(
							"텍스트 파일(*.txt)", "txt"));
					int option = jFileChooser.showSaveDialog(JFileChooserEx.this);
					//어떤 버튼을 클릭했는지 확인하기
					if (option == JFileChooser.APPROVE_OPTION) {
						File file = jFileChooser.getSelectedFile();
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				JFileChooserEx jFrame = new JFileChooserEx();
